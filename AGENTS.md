...
monero.fail is a Monero remote node monitoring web application. Users submit node URLs (scheme://host:port), and the app regularly validates and health-checks them, storing results in SQLite. It also discovers peers via the Levin P2P protocol.
...
xmrnodes/
  ...
  cli.py          - CLI commands: validate, check, export, import, peers
  config.py       - Configuration from environment variables
  models.py       - Peewee models: Node, Peer, HealthCheck
  helpers.py      - HTTP requests, GeoIP, Levin protocol, utility functions
  forms.py        - WTForms for node submission
  filters.py      - Jinja2 template filters
  routes/         - Flask blueprints (meta.py handles /add, node listing, API)
